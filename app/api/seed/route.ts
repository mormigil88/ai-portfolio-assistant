...
Stack: TypeScript, Node.js, Grammy (Telegram), MAX Messenger SDK, Redis FSM, BullMQ queues, Bitrix24 CRM webhooks.`,
  ...
  {
    source: "project-denis-twin",
    content: `Denis Twin — a second, independent product built for the same EcoClub client (a bulk-purchasing bot for Bali expats): a personal AI "digital twin" that captures scattered voice/text notes (previously lost across Telegram/WhatsApp/Gemini/Claude/notes) and files them into a single knowledge base — classifying each into one of 6 types (idea/task/agreement/contact/project/reminder) across 6 business tracks (China sourcing/group buys/investments/marketplace/personal).
Same architecture pattern as the Ashet product line: Claude Code running headless via its Channels mechanism on the client's own Claude Pro subscription (not a platform API key), deployed as a Docker container on Railway with a persistent volume. Voice notes are transcribed via the Groq Whisper API with auto language detection (forcing a language produced garbled "phonetic" output for non-Russian speech in an earlier project, so detection is left open).
Backend gateways live on the shared NeuroStaff orchestrator (Railway), each behind its own per-client secret: a knowledge_gateway (create/list/search entries, overdue/due-today queries) backing morning/evening Telegram summaries sent directly via the Bot API on a cron schedule (skipped entirely when there's nothing to report, to avoid spamming empty digests); and a Google Calendar gateway (OAuth consent flow, refresh-token storage, event creation) that lets the agent create real calendar events from a plain-language message like "meeting with X tomorrow at 3pm."
Found and fixed two real production bugs surfaced only by live client use: (1) the OAuth token-save function passed its SQL parameters as separate positional arguments instead of a tuple, so Postgres silently never stored the token even though the client had completed Google's consent screen; (2) a naive-vs-timezone-aware datetime comparison crashed token-refresh, because the DB column stores UTC timestamps without tzinfo while the comparison code used an explicitly UTC-aware datetime.now() — fixed with an explicit tzinfo=UTC annotation on read.
Designed (not yet built) a chat-history-scanning feature requested directly by the client: Telegram's Bot API can't read history from chats it didn't originate in, so the plan uses a separate service Telegram account (not the client's personal one, to keep blast radius small) logged in via Telethon, restricted to chats it's manually added to, cataloguing links/documents without downloading files — run as a narrow HTTP-gated endpoint on the shared orchestrator rather than an autonomous agent with direct session access.`,
  },
  {
...
Delivered end-to-end in under 48 hours: discovery interview → spec → schema/API/frontend → Railway deploy → live client demo, entirely through conversational iteration with Claude Code (no manual coding).`,
  ...
  {
    source: "project-hr-screening-bot",
    content: `HR Screening Bot — production-architecture MVP for automating candidate pre-screening and interview booking over Telegram, so an HR team only sees candidates who already passed a first filter.
Stack: FastAPI + PostgreSQL + Redis, aiogram Telegram bot, Jinja2 + HTMX admin panel (no separate frontend build), Alembic migrations, Fernet field-level encryption for candidate PII, Docker Compose, pytest test suite.
Candidate flow: GDPR-style consent → phone share → 5–6 configurable pre-screen questions → automatic rules-engine scoring → pass/clarify/soft-reject, with .ics calendar files generated for booked interview slots and automated 24h/3h/45min reminders.
Rules engine is a transparent weighted scorer, not an LLM black box: skills match (70% required-skill coverage + 30% preferred-skill coverage), experience via linear interpolation between a vacancy's minimum and "ideal" years, compensation fit via numeric range-overlap between candidate expectations and the vacancy's salary band, plus availability scoring from shift/start-date answers — each candidate gets a human-readable breakdown ("Хорошее совпадение по навыкам", "Общий балл: 76/100"), not just a number.
Claude (claude-3-haiku) is used narrowly for two things layered on top of the deterministic scorer: structured profile extraction from free-text resumes into a strict JSON schema (explicitly instructed not to invent missing fields — nulls stay null), and a short HR-facing summary (verdict/strengths/risks/salary/availability) — both wrapped in fallbacks that degrade to an empty-but-valid profile or a "needs manual review" message on API failure, so a Claude outage never blocks the pipeline, only removes the summary.
HR side: an inbox with filters, a kanban board across screening stages, per-candidate detail cards (AI summary + raw answers + booking history), a scoring-rules editor, and conversion/no-show analytics — all server-rendered HTMX, no SPA framework needed for an internal tool.`,
  },
  {
    source: "project-avito-neurostaff-tool",
    content: `Avito lead-gen automation — turned a manual workflow (generating ad creative images + filling out Avito's listing form by hand) into a reusable module on the NeuroStaff AI-agent platform, callable by any neuro-employee as a tool function.
avito_creative.py generates ad cover/scheme images (background via the platform's Pollinations.ai image tool, text overlay via PIL/Arial Bold) — tested end-to-end, output verified visually. avito_publish.py drives Avito's listing form with Playwright (the standalone playwright package, not an MCP browser tool) to create a draft listing.
Deliberate structural safety guardrail, not a prompt-level one: the code that clicks Avito's "Разместить" (publish/go-live) button simply does not exist in the module — only "Сохранить и выйти" (save draft) is implemented, so a misfiring agent can create an unpublished draft at worst, never a live paid listing, regardless of what the LLM decides to do.
Runs a separate, isolated browser profile from other Playwright-driven tools on the same machine (job-hunter's LinkedIn automation shares a profile directory that would otherwise conflict) and is scoped as Mac-only by design: NeuroStaff's production agents run headless on Railway, and launching a fresh browser session against Avito from a cloud IP with no prior login history is a straightforward bot-detection trigger, so the tool intentionally only runs against a locally logged-in browser profile, never in production.
Registered as callable tool functions (generate_avito_ad_images, avito_create_draft) in the platform's tool-calling framework, gated behind an explicit per-agent opt-in rather than being auto-enabled — category presets (e.g. ai_services) are parameterized from the start so new Avito categories are a config addition, not a code change.`,
  },
  {
